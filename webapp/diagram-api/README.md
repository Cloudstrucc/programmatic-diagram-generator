...
- **Draw.io Templates**: Pre-configured templates and styles for AWS, Azure, GCP, Kubernetes, Network, and more
- **Multiple Diagram Types**: Support for Mermaid and draw.io formats
...
Draw.io with AWS Template:
```bash
curl -X POST http://localhost:3000/api/diagram/generate \
  -H "Content-Type: application/json" \
  -H "X-API-Key: your-api-key" \
  -d '{
    "prompt": "Create a 3-tier web application with load balancer, EC2 instances, and RDS",
    "diagramType": "drawio",
    "templateType": "aws"
  }'
```
